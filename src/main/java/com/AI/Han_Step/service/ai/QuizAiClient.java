...
public class QuizAiClient {
    ...
    private String buildPrompt(String level, int count) {
        ...
        * “미안해” : 친구/가족 등에게 가볍게 사과하는 표현
        ...
        - WORD_ORDER 문제는 "인사 표현 자체"를 맞추는 문제가 아니다.
        - WORD_ORDER 문제에서는 자연스러운 한국어 문장을 단어로 나누어 순서를 섞어야 한다.
        - 완전한 하나의 문장만 사용해야 하고, 문장 안에서
          "안녕하세요", "안녕히 주무세요", "감사합니다", "죄송합니다", "미안해"
          와 같은 인사/감사 표현을 다른 명사구와 억지로 이어붙이지 마라.
        
          잘못된 예시:
          - "아침에 처음 만났을 때 하는 인사 안녕하세요"
          - "선생님께 사과하는 말 죄송합니다"
        ...
        - question:
          - 배열할 단어는 쓰지 말고, 안내 문장만 적는다.
          - 예: "다음 단어들을 올바른 순서로 배열하여 문장을 만드세요."
        
        - answer:
          - 자연스러운 한 문장을 단어 단위로 나누어 JSON 배열로 제공한다.
          - 예: ["아침에", "처음", "만난", "사람에게", "인사해요"]
        
        - choices:
          - answer 배열과 **동일한 단어들만** 사용하고, 순서만 섞어서 제공한다.
          - 단어를 추가/삭제/변경하면 안 된다.
          - 예: ["사람에게", "인사해요", "처음", "만난", "아침에"]
        
          BEGINNER 레벨의 WORD_ORDER 문제는 다음 규칙을 반드시 따른다.
                
                  1) 문장 구조는 반드시 아래 형태만 사용한다:
                     "[주어]는 [목적어]를 [동사]한다"
                
                  2) 모든 단어는 한국어 기초 수준의 단어만 사용한다.
                
                  3) answer는 위 문장을 단어 단위로 "주어|목적어|동사" 형식으로 제공한다.
                     예: "나는|물을|마신다"
                
                  4) choices는 반드시 answer의 단어들만 포함하고, 순서만 섞는다.
                     예: "마신다|나는|물을"
                
                  5) question에는 단어를 절대 포함하지 않는다.
                     question은 항상 아래 문장으로 고정한다:
                     "다음 단어들을 올바른 순서로 배열하여 문장을 만드세요."
